...
func TestValidate(t *testing.T) {
	tests := []struct {
		...
		{
			in: User{
				ID: "12345678-90ab-1234-5678-0123456789ab", Name: "Somebody", Age: 25, Email: "some@ya.ru", Role: "admin",
				Phones: []string{"79169999999", "79169999998"},
			},
			expectedErr: nil,
		},
		{in: 0, expectedErr: ErrArgumentNotStructure},
		{in: Bad1{Phone: "+79169999999"}, expectedErr: ErrInvalidLenTag},
		{in: Bad2{Age: 10}, expectedErr: ErrInvalidMinTag},
		{in: Bad3{Age: 10}, expectedErr: ErrInvalidMaxTag},
		{in: Bad4{Age: 1}, expectedErr: ErrInvalidInTag},
		{in: Bad5{Age: 1}, expectedErr: ErrInvalidTag},
		{in: Bad6{Phone: "1"}, expectedErr: ErrInvalidRegExp},
		{in: App{Version: "1234"}, expectedErr: ValidationErrors{{Field: "Version", Err: ErrWrongLen}}},
		{in: App{Version: "АБВГД"}, expectedErr: nil},
		{
			in: User{
				ID: "12345678-90ab-1234-5678-0123456789ab", Name: "Somebody", Age: 25, Email: "some#ya.ru", Role: "admin",
				Phones: []string{"79169999999", "79169999998"},
		...
		{
			in: User{
				ID: "12345678-90ab-1234-5678-0123456789ab", Name: "Somebody", Age: 25, Email: "some@ya.ru", Role: "user",
				Phones: []string{"79169999999", "79169999998"},
			},
		...
		{
			in: User{
				ID: "12345678-90ab-1234-5678-0123456789ab", Name: "Somebody", Age: 10, Email: "some@ya.ru", Role: "admin",
				Phones: []string{"79169999999", "79169999998"},
			},
		...
		{
			in: User{
				ID: "12345678-90ab-1234-5678-0123456789ab", Name: "Somebody", Age: 70, Email: "some@ya.ru", Role: "admin",
				Phones: []string{"79169999999", "79169999998"},
			},
			expectedErr: ValidationErrors{{Field: "Age", Err: ErrMaxFailed}},
		},
		{in: Bad7{Age: 10}, expectedErr: ErrUnknownValidator},
		{
			in:          Token{Header: []byte("header"), Payload: []byte("payload"), Signature: []byte("signature")},
			expectedErr: nil,
		},
		{in: Response{Code: 200}, expectedErr: nil},
		{in: Response{Code: 333}, expectedErr: ValidationErrors{{Field: "Code", Err: ErrNotInSet}}},
		{
			in: User{
				ID: "12345678-90ab-1234-5678-0123456789ab", Name: "Somebody", Age: 25, Email: "some@ya.ru", Role: "admin",
				Phones: []string{"1", "12"},
		...
		{
			in: User{
				ID: "1", Name: "Somebody", Age: 70, Email: "some#ya.ru", Role: "ZZZ",
				Phones: []string{"+79169999999", "+79169998"},
			},
		...
		{in: IntArray{Data: []int{77, 9999}}, expectedErr: ValidationErrors{
			{Field: "Data", Err: ErrMinFailed},
			{Field: "Data", Err: ErrMaxFailed},
		}},
		{
			in: Staff{
				User: User{
	...
	for i, tt := range tests {
		t.Run(fmt.Sprintf("case %d", i), func(t *testing.T) {
			...
			if tt.expectedErr == nil {
				...
				checkValidationErrors(t, tt.expectedErr, validationErr)
			}
			_ = tt
		})
	}
...
func checkValidationErrors(t *testing.T, expectedErr error, validationErr error) {
	t.Helper()
	var expectedErrors ValidationErrors
	if errors.As(expectedErr, &expectedErrors) {
		...
		require.ErrorIs(t, validationErr, expectedErr)
	}
}
